"""Source-bound paired study against the current fresh-challenger alpha leader."""
...
class Study:
    FAMILY = "offensive_recovered_provenance_authority"
    REGISTRATION_COMMIT = "8a9d96ea500a3a6eb9b3ab7a17880866c491bf45"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("recovered provenance study only accepts its preregistered family")
        if issued_evidence is not None:
            raise ValueError("recovered provenance study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_recovered_provenance_authority_study.py",
            "offensive_recovered_provenance_authority.py",
            "offensive_recovered_provenance_authority_contract.json",
    ...
    def _saved(self, market, path, owner, *, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected recovered provenance study identity")
    ...
    def select(self, market, catalog, out):
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_recovered_provenance_authority_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != contract["evidence_basis"]["full_data_sha256"]:
            raise ValueError("recovered provenance authority requires frozen full market")
        market = market.prefix(contract["fixed_screen"]["window"][1])
        scoped = scopes(market, catalog)
        out = Path(out)
        plan = {"identity": self.identity(), "full_data_sha256": full_sha,
                "data_sha256": market.fingerprint(), "selection_end": contract["fixed_screen"]["window"][1],
        ...
        for name, names in scoped.items():
            ...
            row["authority_blocks"] = sum(
                event.get("action") == "RECOVERED_PROVENANCE_AUTHORITY_BLOCK"
                for event in owners["treatment"].trace
            )
            rows.append(row)
        ...
        write_json(out / "selection.json", {
            ...
            "advance": advance, "rows": rows, "baseline_objective": baseline,
            "current_leader_objective": leader, "treatment_objective": treatment,
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("recovered provenance selection identity mismatch")
        if not chosen["advance"]:
            write_json(out.parent / "evaluation-decision.json", {
                "status": "NOT_RUN_REJECTED_VS_CURRENT_LEADER", "selection_sha256": file_hash(selection),
                "full_history_runs": 0})
            return
        ...
        for name, names in scopes(market, catalog).items():
            subset = market.subset(names)
            owners = {"current_leader": LeaderOwner(subset, LeaderParameters(True)),
                      "treatment": self.module.Owner(subset, self.module.Parameters(True))}
            for label, owner in owners.items():
                result = self._saved(subset, out / "runs" / f"{name}_{label}", owner)
