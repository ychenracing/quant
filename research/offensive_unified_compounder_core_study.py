"""Source-bound wealth study for the preregistered standalone unified compounder core."""
...
class Study:
    FAMILY = "offensive_unified_compounder_core"
    REGISTRATION_COMMIT = "455f2c3f4e91d6596db1fe6494c69bd5d58af978"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("unified compounder study accepts only its preregistered family")
        if issued_evidence is not None:
            raise ValueError("unified compounder study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_unified_compounder_core_study.py",
            "offensive_unified_compounder_core.py",
            "offensive_unified_compounder_core_contract.json",
    ...
    def _saved(self, market, path, owner, *, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected unified compounder study identity")
    ...
    def select(self, market, catalog, out):
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_unified_compounder_core_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != "d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b":
            raise ValueError("unified compounder core requires the frozen full market")
        ...
        for name, names in scoped.items():
            ...
            if hasattr(owners["treatment"], "trace"):
                trace = owners["treatment"].trace
                row["treatment_events"] = {
                    "proven": sum(r.get("action") == "COMPOUNDER_PROVEN" for r in trace),
                    "ignored_slow_exits": sum(r.get("action") == "PROVEN_SLOW_EXIT_IGNORED" for r in trace),
                    "security_exits": sum(r.get("action") == "SECURITY_EXIT" for r in trace),
                    "displacements": sum(r.get("action") == "ACTIVE_DISPLACEMENT" for r in trace),
                    "peak_blocks": sum(r.get("action") == "FUNDED_PEAK_DISPLACEMENT_BLOCK" for r in trace),
                    "reference_rearms": sum(r.get("action") == "REFERENCE_ALPHA_REARM" for r in trace),
                }
            rows.append(row)
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("unified compounder selection identity mismatch")
