"""Source-bound wealth study for the preregistered positive-alpha compounder."""
...
class Study:
    FAMILY = "offensive_positive_alpha_compounder"
    REGISTRATION_COMMIT = "a5c41101bbcc1dfead12eff17d3da7033fc9f5bd"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("positive alpha compounder study accepts only its preregistered family")
        if issued_evidence is not None:
            raise ValueError("positive alpha compounder study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_positive_alpha_compounder_study.py",
            "offensive_positive_alpha_compounder.py",
            "offensive_positive_alpha_compounder_contract.json",
    ...
    def _saved(self, market, path, owner, *, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected positive alpha compounder study identity")
    ...
    def select(self, market, catalog, out):
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_positive_alpha_compounder_contract.json").read_text())
        full_sha = market.fingerprint()
        # Same frozen market identity used by the campaign-peak canonical run.
        if full_sha != "d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b":
            raise ValueError("positive alpha compounder requires the frozen full market")
        ...
        advance = (
            treatment["mean_log"] > champion["mean_log"] + 1e-12
            and treatment["minimum"] >= champion["minimum"] - 1e-12
        )
        decision = {
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("positive alpha compounder selection identity mismatch")
