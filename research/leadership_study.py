"""Matched, finite leadership study. No ordinary CI or acceptance overrides."""
...
from research.leadership import Owner, Parameters
...
def identity():
    return {'source':source_identity(),'runner_sha256':file_hash(Path(__file__)),
            ...
            'policy_sha256':file_hash(Path(__file__).with_name('leadership.py'))}
...
def select(market,catalog,out):
    ...
    grid=[Parameters(*v) for v in itertools.product((60,120,252),(20,60),(2,4),(False,True))]
    plan={'identity':identity(),'data_sha256':market.fingerprint(),'selection_end':'2025-12-31',
          'grid':[asdict(p) for p in grid],'scopes':scope_map}
    ...
    for index,p in enumerate(grid):
        label=f'lead{p.leadership_window}_exit{p.exit_span}_slots{p.positions}_pressure{int(p.pressure_guard)}'
...
def evaluate(market,catalog,selection,out):
    ...
    for name,symbols in scopes(market,catalog).items():
        m=market.subset(symbols)
        for policy in ('leadership','incumbent','buy_hold'):
            result=saved(m,out/'runs'/(name+'_'+policy),params if policy=='leadership' else None,
