"""Source-bound paired study for the preregistered campaign-epoch reset family."""
...
class Study:
    FAMILY = "offensive_campaign_epoch_reset"
    REGISTRATION_COMMIT = "825b0f9134f87403cde971ccfe5e3977f7007183"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("campaign epoch study only accepts its preregistered family")
        if issued_evidence is not None:
            raise ValueError("campaign epoch study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_campaign_epoch_reset_study.py",
            "offensive_campaign_epoch_reset.py",
            "offensive_campaign_epoch_reset_contract.json",
    ...
    def saved(self, market, path, parameters, benchmark=None, costs=1.0, delay=1):
        ...
        result = run(market, cfg, benchmark=benchmark, policy_factory=lambda _m, _c: owner,
                     cost_multiplier=costs, delay=delay)
        result.metadata["study"] = self.identity()
        if result.metadata != expected:
            raise AssertionError("unexpected campaign epoch study identity")
    ...
    def _trace_summary(trace):
        ...
        epoch = [r for r in trace if r.get("kind") == "CAMPAIGN_EPOCH_RESET_EVENT"]
        return {
            ...
            "invalidations": sum(r.get("action") == "ACUTE_CAMPAIGN_INVALIDATION" for r in epoch),
            "rearms": sum(r.get("action") == "FRESH_EPOCH_REARM" for r in epoch),
            "exit_retries": sum(r.get("action") == "INVALIDATED_EXIT_RETRY" for r in epoch),
        }
    ...
    def select(self, market, catalog, out):
        ...
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_campaign_epoch_reset_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != contract["frozen_inputs"]["full_sha256"]:
            raise ValueError("campaign epoch reset requires the frozen full market")
        ...
        if (out / "plan.json").exists() and json.loads((out / "plan.json").read_text()) != plan:
            raise ValueError("existing campaign epoch plan is not equivalent")
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if (chosen["identity"] != self.identity()
                ...
                or chosen["plan_sha256"] != file_hash(Path(selection).with_name("plan.json"))):
            raise ValueError("campaign epoch selection identity mismatch")
        if not chosen["advance"]:
            write_json(out.parent / "evaluation-decision.json", {
                "status": "NOT_RUN_REJECTED_PAIR", "selection_sha256": file_hash(Path(selection)),
        ...
        write_json(out / "plan.json", {
            ...
            "selection_sha256": file_hash(Path(selection)), "parameters": asdict(self.module.Parameters(True)),
            "windows": windows,
        })
        rows = []
