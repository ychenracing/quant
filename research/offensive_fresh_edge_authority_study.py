...
class Study:
    FAMILY = "offensive_fresh_edge_authority"
    REGISTRATION_COMMIT = "c7b85df7458f9427d35ca7831d345744f3922ad5"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("fresh edge study only accepts its preregistered family")
        if issued_evidence is not None:
            raise ValueError("fresh edge study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_fresh_edge_authority_study.py",
            "offensive_fresh_edge_authority.py",
            "offensive_fresh_edge_authority_contract.json",
    ...
    def _saved(self, market, path, owner, *, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected fresh edge study identity")
    ...
    def select(self, market, catalog, out):
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_fresh_edge_authority_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != contract["evidence_basis"]["full_data_sha256"]:
            raise ValueError("fresh edge authority requires frozen full market")
        ...
        for name, names in scoped.items():
            ...
            row["authority_blocks"] = sum(
                event.get("action") == "FRESH_EDGE_AUTHORITY_BLOCK"
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("fresh edge selection identity mismatch")
