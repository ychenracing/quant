"""Source-bound wealth study for the preregistered single dominant campaign."""
...
class Study:
    ...
    def _saved(self,market,path,owner):
        ...
        if result.metadata!=expected: raise AssertionError("unexpected single dominant campaign study identity")
    ...
    def select(self,market,catalog,out):
        ...
        if full_sha!="d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b": raise ValueError("single dominant campaign requires frozen full market")
        market=market.prefix("2025-12-31"); scoped=scopes(market,catalog); out=Path(out); write_json(out/"plan.json",{"identity":self.identity(),"full_data_sha256":full_sha,"data_sha256":market.fingerprint(),"selection_end":"2025-12-31","scopes":scoped,"registration_commit":self.REGISTRATION_COMMIT,"contract_sha256":file_hash(root/"offensive_single_dominant_campaign_contract.json")})
        rows=[]
        for name,names in scoped.items():
            ...
            held_days=int((treatment.parent.parent.base.config.max_positions==1)*len(subset.calendar)); row["architecture"]={"max_positions":1,"sessions":held_days}; rows.append(row); write_json(out/"paired-progress.json",rows); print(json.dumps(row),flush=True)
    ...
    def evaluate(self,market,catalog,selection,out):
        ...
        if chosen["identity"]!=self.identity() or chosen["full_data_sha256"]!=market.fingerprint(): raise ValueError("single dominant selection identity mismatch")
