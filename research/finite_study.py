...
class Study:
    def __init__(self,family:str,*,issued_evidence:Path|None=None):
        if family not in {'shock_ownership','nonlinear','observed_trend','pathwise','coherent','trend_book','recovery_memory','support_budget','risk_reliability','funded_risk','quantity_obligation','observed_readiness'}:raise ValueError('undeclared research family')
        self.family=family
    ...
    def identity(self):
        ...
        if self.family=='observed_readiness':names+=['quantity_obligation.py','quantity_obligation_contract.json']
        if self.family in {'quantity_obligation','observed_readiness'}:names+=['support_budget.py','support_budget_contract.json','funded_risk.py','funded_risk_contract.json']
    ...
    def saved(self,market,path,parameters=None,benchmark=None,costs=1.,delay=1):
        ...
        if path.exists():
            if self.family in {'quantity_obligation','observed_readiness'} and owner is not None:
