...
class Study:
    ...
    def identity(self):
        ...
        if self.family in {'observed_readiness','leadership_handover','opportunity_reassignment','admission_budget_completion','observed_admission_completion','funded_reentry','revocable_reentry','suspended_campaign','allocation_auction','admission_structure','protected_retention','healthy_hold_funding','opportunity_allocation','continuity_selection'}:names+=['quantity_obligation.py','quantity_obligation_contract.json']
        if self.family in {'quantity_obligation','observed_readiness','leadership_handover','opportunity_reassignment','admission_budget_completion','observed_admission_completion','funded_reentry','revocable_reentry','suspended_campaign','allocation_auction','admission_structure','protected_retention','healthy_hold_funding','opportunity_allocation','continuity_selection'}:names+=['support_budget.py','support_budget_contract.json','funded_risk.py','funded_risk_contract.json']
        if self.family in {'observed_admission_completion','funded_reentry','revocable_reentry','suspended_campaign','allocation_auction','admission_structure','protected_retention','healthy_hold_funding','opportunity_allocation','continuity_selection'}:
        ...
        if self.family in {'allocation_auction','admission_structure','protected_retention','healthy_hold_funding','opportunity_allocation','continuity_selection'}:names+=['observed_admission_completion.py','observed_admission_completion_contract.json']
        if self.family=='admission_structure':names+=['admission_geometry.py','ledger_attribution.py','records/admission_structure_observations.json','records/admission_geometry_manifest.json']
        if self.family=='protected_retention':names+=['retention_diagnosis.py','ledger_attribution.py','admission_geometry.py','records/protected_retention_observations.json','records/admission_structure_observations.json','records/admission_geometry_manifest.json']
        if self.family=='healthy_hold_funding':names+=['decision_review.py']
        if self.family in {'opportunity_allocation','continuity_selection'}:names+=['decision_review.py','opportunity_allocation_study.py','ledger_attribution.py']
    ...
    def saved(self,market,path,parameters=None,benchmark=None,costs=1.,delay=1,*,configuration=None):
        if configuration is not None and (type(configuration) is not Config
                or self.family not in {"observed_admission_completion", "allocation_auction", "admission_structure", "protected_retention", "healthy_hold_funding", "opportunity_allocation", "continuity_selection"}):
        ...
        if path.exists():
            if self.family in {'quantity_obligation','observed_readiness','leadership_handover','opportunity_reassignment','admission_budget_completion','observed_admission_completion','funded_reentry','revocable_reentry','suspended_campaign','allocation_auction','admission_structure','protected_retention','healthy_hold_funding','opportunity_allocation','continuity_selection'} and owner is not None:
