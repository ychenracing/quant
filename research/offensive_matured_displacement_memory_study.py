"""Source-bound wealth study for the preregistered matured displacement memory."""
...
class Study:
    FAMILY = "offensive_matured_displacement_memory"
    REGISTRATION_COMMIT = "60e97741aae04b7cc7adf158355fee96e3fb2667"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("matured displacement memory study accepts only its preregistered family")
        if issued_evidence is not None:
            raise ValueError("matured displacement memory study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_matured_displacement_memory_study.py",
            "offensive_matured_displacement_memory.py",
            "offensive_matured_displacement_memory_contract.json",
    ...
    def _saved(self, market, path, owner):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected matured displacement memory study identity")
    ...
    def select(self, market, catalog, out):
        ...
        if full_sha != "d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b":
            raise ValueError("matured displacement memory requires frozen full market")
        market = market.prefix("2025-12-31")
        scoped = scopes(market, catalog)
        out = Path(out)
        write_json(out / "plan.json", {"identity": self.identity(), "full_data_sha256": full_sha, "data_sha256": market.fingerprint(), "selection_end": "2025-12-31", "scopes": scoped, "registration_commit": self.REGISTRATION_COMMIT, "contract_sha256": file_hash(root / "offensive_matured_displacement_memory_contract.json")})
        rows = []
        for name, names in scoped.items():
            ...
            row["memory"] = {
                "registered_events": len(treatment.pending_events),
                "matured_events": int(treatment.matured_count),
                "final_mean": treatment.memory_mean,
                "allow_events": sum(r.get("action") == "MEMORY_ALLOW_DISPLACEMENT" for r in treatment.trace),
                "block_events": sum(r.get("action") == "MEMORY_BLOCK_DISPLACEMENT" for r in treatment.trace),
                "fallback_peak_blocks": sum(r.get("action") == "MEMORY_FALLBACK_CAMPAIGN_PEAK_BLOCK" for r in treatment.trace),
            }
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint(): raise ValueError("matured displacement memory selection identity mismatch")
