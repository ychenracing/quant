"""Source-bound wealth study for the preregistered dominant-peak authority."""
...
class Study:
    ...
    def _saved(self,market,path,owner):
        ...
        if result.metadata!=expected: raise AssertionError("unexpected dominant peak authority study identity")
    ...
    def select(self,market,catalog,out):
        ...
        if full_sha!="d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b": raise ValueError("dominant peak authority requires frozen full market")
        market=market.prefix("2025-12-31"); scoped=scopes(market,catalog); out=Path(out); write_json(out/"plan.json",{"identity":self.identity(),"full_data_sha256":full_sha,"data_sha256":market.fingerprint(),"selection_end":"2025-12-31","scopes":scoped,"registration_commit":self.REGISTRATION_COMMIT,"contract_sha256":file_hash(root/"offensive_dominant_peak_authority_contract.json")})
        rows=[]
        for name,names in scoped.items():
            ...
            events=treatment.trace; row["authority"]={"dominant_blocks":sum(r.get("action")=="DOMINANT_FUNDED_PEAK_DISPLACEMENT_BLOCK" for r in events),"weaker_peak_releases":sum(r.get("action")=="WEAKER_PEAK_RELEASE" for r in events)}; rows.append(row); write_json(out/"paired-progress.json",rows); print(json.dumps(row),flush=True)
    ...
    def evaluate(self,market,catalog,selection,out):
        ...
        if chosen["identity"]!=self.identity() or chosen["full_data_sha256"]!=market.fingerprint(): raise ValueError("dominant peak selection identity mismatch")
