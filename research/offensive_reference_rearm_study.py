"""Source-bound paired study for the preregistered reference-rearm family."""
...
class Study:
    FAMILY = "offensive_reference_rearm"
    REGISTRATION_COMMIT = "f63c1d52939eaa08b31553b646035278392f7cfe"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("reference rearm study only accepts its preregistered family")
        if issued_evidence is not None:
            raise ValueError("reference rearm study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_reference_rearm_study.py",
            "offensive_reference_rearm.py",
    ...
    def saved(self, market, path, parameters, benchmark=None, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected reference rearm study identity")
    ...
    def _trace_summary(trace):
        ...
        rearm = [r for r in trace if r.get("kind") == "REFERENCE_REARM_EVENT"]
        return {
    ...
    def select(self, market, catalog, out):
        ...
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_reference_rearm_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != contract["frozen_inputs"]["full_sha256"]:
            raise ValueError("reference rearm requires the frozen full market")
        ...
        if (out / "plan.json").exists() and json.loads((out / "plan.json").read_text()) != plan:
            raise ValueError("existing reference rearm plan is not equivalent")
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if (
            ...
            raise ValueError("reference rearm selection identity mismatch")
