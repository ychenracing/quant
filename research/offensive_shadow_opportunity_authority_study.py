...
class Study:
    FAMILY = "offensive_shadow_opportunity_authority"
    REGISTRATION_COMMIT = "c7ecdc0e00acaaf93dd06d3d07eacf1fc9b99ed4"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("shadow opportunity study only accepts its preregistered family")
        if issued_evidence is not None:
            raise ValueError("shadow opportunity study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_shadow_opportunity_authority_study.py",
            "offensive_shadow_opportunity_authority.py",
            "offensive_shadow_opportunity_authority_contract.json",
    ...
    def _saved(self, market, path, owner, *, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected shadow opportunity study identity")
    ...
    def select(self, market, catalog, out):
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_shadow_opportunity_authority_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != contract["frozen_inputs"]["full_sha256"]:
            raise ValueError("shadow opportunity authority requires frozen full market")
        ...
        plan = {
            ...
            "data_sha256": market.fingerprint(), "selection_end": contract["measurement"]["window"]["end"],
        ...
        advance = (
            treatment["mean_log"] > leader["mean_log"] + 1e-12
            and treatment["minimum"] >= leader["minimum"] - 1e-12
        )
        decision = {
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("shadow opportunity selection identity mismatch")
