"""Matched, finite admission study. No ordinary CI or acceptance overrides."""
...
from research.admission import Owner, Parameters, grid
...
def identity():
    return {'source':source_identity(),'runner_sha256':file_hash(Path(__file__)),
            ...
            'policy_sha256':file_hash(Path(__file__).with_name('admission.py')),
...
def select(market,catalog,out):
    ...
    for index,p in enumerate(candidates):
        label=f'{p.leader_rule}_market{p.market_span}_breadth{p.minimum_breadth:g}'
...
def evaluate(market,catalog,selection,out):
    ...
    for name,symbols in scopes(market,catalog).items():
        m=market.subset(symbols)
        for policy in ('admission','incumbent','buy_hold'):
            result=saved(m,out/'runs'/(name+'_'+policy),params if policy=='admission' else None,
