...
class Study:
    FAMILY = "offensive_campaign_peak_authority"
    REGISTRATION_COMMIT = "4842253f8fad15e92fcfaf1e6fce6b282353bf9e"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("campaign peak study only accepts its preregistered family")
        if issued_evidence is not None:
            raise ValueError("campaign peak study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_campaign_peak_authority_study.py",
    ...
    def _saved(self, market, path, owner, *, costs=1.0, delay=1):
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected campaign peak study identity")
    ...
    def select(self, market, catalog, out):
        root = Path(__file__).parent
        contract = json.loads((root / "offensive_campaign_peak_authority_contract.json").read_text())
        full_sha = market.fingerprint()
        if full_sha != contract["evidence_basis"]["full_data_sha256"]:
            raise ValueError("campaign peak authority requires frozen full market")
        ...
        for name, names in scoped.items():
            ...
            row["authority_blocks"] = sum(
                event.get("action") == "CAMPAIGN_PEAK_AUTHORITY_BLOCK"
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("campaign peak selection identity mismatch")
