"""Source-bound wealth study for the preregistered long-horizon nonlinear alpha."""
...
class Study:
    ...
    REGISTRATION_COMMIT = "f2d7baf822f753b6b7d36ffeb1b73ef118293d31"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("long horizon nonlinear alpha study accepts only its preregistered family")
        if issued_evidence is not None:
            raise ValueError("long horizon nonlinear alpha study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_long_horizon_nonlinear_alpha_study.py",
            "offensive_long_horizon_nonlinear_alpha.py",
            "offensive_long_horizon_nonlinear_alpha_contract.json",
            "offensive_campaign_peak_authority.py", "offensive_campaign_peak_authority_contract.json",
            "offensive_fresh_challenger_authority.py", "offensive_fresh_challenger_authority_contract.json",
            "offensive_alpha_decay_displacement.py", "offensive_alpha_decay_displacement_contract.json",
            "finite_study/__init__.py", "trend_book.py", "trend_book_contract.json", "coherent.py", "coherent_contract.json",
            ...
            "quantity_obligation.py", "quantity_obligation_contract.json", "support_budget.py", "support_budget_contract.json",
            "funded_risk.py", "funded_risk_contract.json", "observed_readiness.py", "observed_readiness_contract.json",
            "admission_budget_completion.py", "admission_budget_completion_contract.json", "observed_admission_completion.py",
            "observed_admission_completion_contract.json", "decision_review.py", "ledger_attribution.py",
        ]
    ...
    def _saved(self, market, path, owner):
        ...
        expected = {
            ...
            "delay": 1, "cost_multiplier": 1.0, "benchmark": None,
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected long horizon nonlinear alpha study identity")
    ...
    def _objective(rows, label):
        ...
        return {"mean_log": math.fsum(math.log(v) for v in wealth) / len(wealth), "minimum": min(wealth), "wealth": {r["scope"]: float(r[label]["wealth"]) for r in rows}}
    ...
    def select(self, market, catalog, out):
        ...
        if full_sha != "d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b":
            raise ValueError("long horizon nonlinear alpha requires frozen full market")
        market = market.prefix("2025-12-31")
        scoped = scopes(market, catalog)
        out = Path(out)
        write_json(out / "plan.json", {"identity": self.identity(), "full_data_sha256": full_sha, "data_sha256": market.fingerprint(), "selection_end": "2025-12-31", "scopes": scoped, "registration_commit": self.REGISTRATION_COMMIT, "contract_sha256": file_hash(root / "offensive_long_horizon_nonlinear_alpha_contract.json")})
        rows = []
        for name, names in scoped.items():
            subset = market.subset(names)
            champion = ChampionOwner(subset, ChampionParameters(True))
            ...
            correlations = [r["training_rank_correlation"] for r in treatment.audit["refits"] if r["training_rank_correlation"] is not None]
            row["learning"] = {
                "fit_count": int(treatment.audit["fit_count"]),
                "training_samples": int(treatment.audit["training_samples"]),
                "last_refit_session": int(treatment.audit["refits"][-1]["refit_session"]) if treatment.audit["refits"] else None,
                "last_rank_correlation": correlations[-1] if correlations else None,
                "median_rank_correlation": float(pd.Series(correlations).median()) if correlations else None,
            }
            rows.append(row); write_json(out / "paired-progress.json", rows); print(json.dumps(row), flush=True)
        champion_obj = self._objective(rows, "current_champion"); treatment_obj = self._objective(rows, "treatment")
        advance = treatment_obj["mean_log"] > champion_obj["mean_log"] + 1e-12 and treatment_obj["minimum"] >= champion_obj["minimum"] - 1e-12
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint(): raise ValueError("long horizon nonlinear alpha selection identity mismatch")
