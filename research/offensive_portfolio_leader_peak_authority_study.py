"""Source-bound wealth study for portfolio-leader peak release authority."""
...
class Study:
    FAMILY = "offensive_portfolio_leader_peak_authority"
    REGISTRATION_COMMIT = "512997c846d7fe7798e2c599604de928c07a2579"

    def __init__(self, family: str, *, issued_evidence=None):
        if family != self.FAMILY:
            raise ValueError("portfolio leader peak study accepts only its preregistered family")
        if issued_evidence is not None:
            raise ValueError("portfolio leader peak study does not accept issued forecasts")
        self.family = family
    ...
    def identity(self):
        ...
        names = [
            "offensive_portfolio_leader_peak_authority_study.py",
            "offensive_portfolio_leader_peak_authority.py",
            "offensive_portfolio_leader_peak_authority_contract.json",
            "offensive_dominant_peak_authority.py",
            "offensive_dominant_peak_authority_contract.json",
            ...
            "trend_book.py",
            "trend_book_contract.json",
            "coherent.py",
            "coherent_contract.json",
            "observed_trend.py",
            "observed_trend_contract.json",
            "nonlinear.py",
            "nonlinear_contract.json",
            "pathwise.py",
            "pathwise_contract.json",
            "expectation_study.py",
            "leadership.py",
            "expectation.py",
            "quantity_obligation.py",
            "quantity_obligation_contract.json",
            "support_budget.py",
            "support_budget_contract.json",
            "funded_risk.py",
            "funded_risk_contract.json",
            "observed_readiness.py",
            "observed_readiness_contract.json",
            "admission_budget_completion.py",
            "admission_budget_completion_contract.json",
            "observed_admission_completion.py",
            "observed_admission_completion_contract.json",
            "decision_review.py",
        ...
        return {
            ...
            "family": self.family,
            "dependencies": {n: file_hash(root / n) for n in names},
        }
    ...
    def _saved(self, market, path, owner):
        ...
        expected = {
            ...
            "cost_multiplier": 1.0,
            "benchmark": None,
            "start": str(market.calendar[0].date()),
            "end": str(market.calendar[-1].date()),
            "economic_acceptance": "UNVERIFIED",
            "accounting": "adjusted economic units, not actual shares",
            "study": self.identity(),
            "policy": owner.identity(),
        }
        if path.exists():
        ...
        if result.metadata != expected:
            raise AssertionError("unexpected portfolio leader peak study identity")
        save_result(result, path)
        return result

    @staticmethod
    def _objective(rows, label):
        wealth = [float(r[label]["wealth"]) for r in rows]
        return {
            "mean_log": math.fsum(math.log(v) for v in wealth) / len(wealth),
            "minimum": min(wealth),
            "wealth": {r["scope"]: float(r[label]["wealth"]) for r in rows},
        }
    ...
    def select(self, market, catalog, out):
        ...
        if full_sha != "d9ded1d933f292933aa09145bc36a5696421c43dbf266fce018a34ef0b9f3f9b":
            raise ValueError("portfolio leader peak authority requires frozen full market")
        ...
        for name, names in scoped.items():
            ...
            events = treatment.trace
            row["authority"] = {
                "portfolio_leader_releases": sum(
                    r.get("action") == "PORTFOLIO_LEADER_WEAKER_PEAK_RELEASE" for r in events
                ),
                "nonleader_peak_blocks": sum(
                    r.get("action") == "NONLEADER_CHALLENGER_PEAK_BLOCK" for r in events
                ),
                "parent_dominant_blocks": sum(
                    r.get("action") == "DOMINANT_FUNDED_PEAK_DISPLACEMENT_BLOCK" for r in events
                ),
                "parent_weaker_releases": sum(
                    r.get("action") == "WEAKER_PEAK_RELEASE" for r in events
                ),
            }
            rows.append(row)
        ...
        champion_obj = self._objective(rows, "current_champion")
        treatment_obj = self._objective(rows, "treatment")
        advance = (
            treatment_obj["mean_log"] > champion_obj["mean_log"] + 1e-12
            and treatment_obj["minimum"] >= champion_obj["minimum"] - 1e-12
        )
        write_json(
            out / "selection.json",
            {
                ...
                "advance": advance,
                "rows": rows,
                "current_champion_objective": champion_obj,
                "treatment_objective": treatment_obj,
                "identity": self.identity(),
                "data_sha256": market.fingerprint(),
                "full_data_sha256": full_sha,
    ...
    def evaluate(self, market, catalog, selection, out):
        ...
        if chosen["identity"] != self.identity() or chosen["full_data_sha256"] != market.fingerprint():
            raise ValueError("portfolio leader peak selection identity mismatch")
        if not chosen["advance"]:
            write_json(
                out.parent / "evaluation-decision.json",
                {
