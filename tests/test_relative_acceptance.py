"""Dynamic relative acceptance must be exact-case, four-reference, and fail closed."""
...
def identity(layer: str = "Native", **changes):
    value = {
        ...
        "initial_positions": {},
        "seed": 17,
        "costs": {"commission_bps": 2.5, "stamp_schedule": "frozen"},
        "slippage": {"bps": 10.0},
        "delay": 1,
        ...
        "runtime_identity": {"python": "3.13", "numpy": "2.3.5", "pandas": "2.2.3"},
    }
...
class RelativeAcceptanceTests(unittest.TestCase):
    ...
    def test_return_may_tie_but_drawdown_may_not_tie(self):
        ...
        self.assertFalse(tied_drawdown["drawdown_pass"])
        self.assertFalse(tied_drawdown["case_pass"])
    ...
    def test_reference_incomplete_is_never_a_pass(self):
        ...
        self.assertEqual(result["reference_incomplete_cases"], ["case-a"])
    ...
    def test_layers_and_case_identities_cannot_be_mixed(self):
        ...
        value = record()
        value["references"]["trae"]["case_identity_hash"] = case_identity_hash(changed)
    ...
    def test_every_identity_field_changes_the_hash(self):
        ...
        for field in base:
            ...
            with self.subTest(field=field):
                self.assertNotEqual(case_identity_hash(changed), baseline)

    def test_matrix_uses_joint_worst_case_not_average(self):
        passing = record()
        ...
        self.assertAlmostEqual(result["min_return_ratio"], 3.99 / 4.0)

    def test_required_layer_must_have_a_mandatory_case(self):
        optional = record(layer="Corrected", mandatory=False)
        result = evaluate_matrix([optional], required_layers=["Native"])
        self.assertEqual(result["economic_acceptance"], "NOT_MET")
        self.assertEqual(result["required_layers_without_mandatory_cases"], ["Native"])

    def test_duplicate_case_evidence_and_nonfinite_values_fail_closed(self):
        value = record()
        with self.assertRaisesRegex(AcceptanceError, "duplicate case evidence"):
